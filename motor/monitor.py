...
def read_u16(motor, addr, label):
    try:
        ...
        print(f"[OK] {label:<22} addr=0x{addr:04X} -> {value}")
        return value
    except Exception as e:
        print(f"[FAIL] {label:<22} addr=0x{addr:04X} -> {e}")
        return None
...
def read_i32(motor, addr, label):
    try:
        ...
        if value >= (1 << 31):
            value -= (1 << 32)
        print(f"[OK] {label:<22} addr=0x{addr:04X} regs={regs} -> {value}")
        return value
    except Exception as e:
        print(f"[FAIL] {label:<22} addr=0x{addr:04X} -> {e}")
        return None
...
def decode_fault(code):
    ...
    if code == 0:
        ...
        print("그래도 빨간 불이면 경고 상태이거나, 표시 램프가 알람 이력/경고를 나타낼 수 있습니다.")
    elif code == 122:
        print("Er.122 가능성: 절대 위치 모드 제품 매칭 고장")
        print("메뉴얼: 절대값 모터 모델 불일치 또는 모터 모델 설정 오류 가능성.")
    elif code == 136:
        print("Er.136 가능성: 모터 ROM 데이터 검증 오류 또는 파라미터 미저장")
        print("메뉴얼: 모터/드라이버 모델 확인, 엔코더선 연결 확인, 노이즈 확인 필요.")
    elif code in [134, 135]:
        print("Er.A34/Er.A35 계열 가능성: 엔코더 통신/피드백/Z신호 이상")
        print("엔코더 케이블, 커넥터, 배터리 연결을 확인하세요.")
    else:
        print("이 코드의 정확한 의미는 드라이버 표시창의 Er.xxx와 대조해야 합니다.")
